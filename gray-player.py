...
class TSQueue:
    ...
    def __init__(self, size):
        ...
        self.is_empty = threading.Semaphore(size)
        self.lock = threading.Lock()

    def get(self):
        if d: print('%sget> acquired lock' % self.zone)
        self.full.acquire()
        if d: print('%sget> called.' % self.zone)
        self.lock.acquire()
    ...
    def empty(self):
        if d: print('%s>is_empty> called.' %self.zone)
        self.lock.acquire()
        if d: print('%s>is_empty> lock acquired.' %self.zone)
        _empty = not self.l
        self.lock.release()
        if d: print('%s>is_empty> lock released.' %self.zone)
        return _empty
...
#TCP File transfer locking entire ft process, instead of transfer then write, keep a DS of what files are open and lock around it instead.
